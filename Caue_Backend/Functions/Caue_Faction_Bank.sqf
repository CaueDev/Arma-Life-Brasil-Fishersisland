...
['Caue_FactionBank_Withdraw', {
	...
	private _factionVar = [(player getVariable ["faction", "citizen"])] call Caue_FactionSetup_GetBudget;
	if ((server getVariable [_factionVar,0]) < _amount) exitWith {["Sua facção não possui essa quantia.",Color_Red] call A3PL_HUD_Notification;};
    ...
    [format["Você sacou $%1 da conta bancaria da sua facção.",([_amount] call Caue_Lib_NumberText)],Color_Green] call A3PL_HUD_Notification;
...
['Caue_FactionBank_Deposit', {
    ...
    [format["Você depositou $%1 na conta bancária da sua facção.",([_amount] call Caue_Lib_NumberText)],Color_Green] call A3PL_HUD_Notification;
...
['Caue_FactionBank_Transfer', {
    ...
    [format["%2 transferiu $%1 para sua conta bancária com dinheiro da facção.",([_amount] call Caue_Lib_NumberText),(player getVariable ["name","error"])],Color_Green] remoteExecCall ["A3PL_HUD_Notification",_target];
